...
Iteracje rozpoczyna Ingress Agent, który pobiera dane z Kubernetes (rys \ref{fig:53-scr-1}). Następnie aktualizuje on status obiektu API \texttt{lola-demux} (rys. \ref{fig:53-scr-2}), który wykonuje swoje workflow akcji (rys. \ref{fig:53-scr-3}), skutkujące aktualizacją statusu obiektów API \texttt{upf1} i \texttt{upf2} (rys. \ref{fig:53-scr-6}). To skutkuje wywołaniem Operatora dla tych obiektów. Operator wykonuje dla obu workflow akcji (rys. \ref{fig:53-scr-4}). Na koniec oba elementy uderzają w endpoint \texttt{api/data} serwowany przez Agenta Egress (rys. \ref{fig:53-scr-5}).
